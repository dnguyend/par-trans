...
* We provide an $O(nd^2)$ algorithm for short transport time, and $O(td^3)$ for long transport time for the Stiefel manifold $St_{n, d}$ with a family of metrics defined by a parameter $\alpha$ as in [1], (defined previously in [2] with a different parametrization). This contributes to a problem raised in [1].
